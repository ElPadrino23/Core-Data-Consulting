...
// RF-05: Validar o rechazar un documento del cliente (con motivo opcional)
module.exports.ValidarDocumento = async (req, res) => {
    try {
        const { idDocumento, estado, motivo } = req.body;
        ...
        // Si es rechazo, el motivo es obligatorio
        if (estado === 'Rechazado' && (!motivo || motivo.trim() === '')) {
            return res.status(400).json({ exito: false, error: 'Debe indicar el motivo del rechazo' });
        }

        const resultado = await modelClientes.ActualizarEstadoDocumento(idDocumento, estado, motivo || null);

        if (resultado.exito) {
            modelHistorial.RegistrarAccion({
                accion:     'Validar documento (' + estado + ')',
                entidad:    'documentocliente',
                entidad_id: idDocumento,
                ip_origen:  req.ip
            });
            res.json({ exito: true, mensaje: 'Documento ' + estado.toLowerCase() + ' correctamente' });
        } else {
            res.status(400).json({ exito: false, error: resultado.error });
        }
    } catch (error) {
